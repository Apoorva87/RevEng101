...
class RequestInterceptor {
  constructor({ ruleEngine, trafficStore }) {
    ...
    // Listen for forward/drop events from dashboard
    trafficStore.on('forward', (entryId, modifications) => {
      this._resolve(entryId, modifications);
    });
    trafficStore.on('drop', (entryId) => {
      this._reject(entryId);
    });
  }

  async checkRequest(entry, { method, path, headers, body }) {
    if (!this.enabled) return null;

    const matchedRule = this.ruleEngine.matchRequest(entry);
  ...
  _reject(entryId) {
    ...
    p.reject(new Error('Dropped by user'));
  }
